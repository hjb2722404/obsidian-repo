...
[微前端框架 之 single-spa 从入门到精通](https://juejin.im/post/6862661545592111111) 通过 `示例项目 => 打包部署 => 源码分析 => 手写框架` 详细介绍了 `single-spa`。
...
如果你很了解 `single-spa` 或者阅读过 [微前端框架 之 single-spa 从入门到精通](https://juejin.im/post/6862661545592111111) ，你会发现 `single-spa` 就做了两件事，加载微应用（加载方法还是用户自己提供的）、维护微应用状态（初始化、挂载、卸载）。了解多了会发现 `single-spa` 虽好，但是却存在一些比较严重的问题
...
项目发布以后出现了 `bug` ，修复之后需要更新上线，为了清除浏览器缓存带来的应用，一般文件名会带上 `chunkcontent`，微应用发布之后文件名都会发生变化，这时候还需要更新主应用中微应用配置，然后重新编译主应用然后发布，这套操作简直是不能忍受的，这也是 [微前端框架 之 single-spa 从入门到精通](https://juejin.im/post/6862661545592111111) 这篇文章中示例项目中微应用发布时的环境配置选择 `development` 的原因。
...
`样式沙箱` 通过重写 `DOM` 操作方法，来劫持动态样式和 `JS` 脚本的添加，让样式和脚本添加到正确的地方，即主应用的插入到主应用模版内，微应用的插入到微应用模版，并且为劫持的动态样式做了 `scoped css` 的处理，为劫持的脚本做了 `JS` 隔离的处理，更加具体的内容可继续往下阅读或者直接阅读 [qiankun 2.x 运行时沙箱 源码分析](https://juejin.im/post/6885214342552223757)
...
- [微前端框架 之 qiankun 从入门到精通](https://juejin.im/post/6885211340999229454)，文章由以下三部分组成
...
- [qiankun 2.x 运行时沙箱 源码分析](https://juejin.im/post/6885214342552223757)，详细解读了 qiankun 2.x 版本的沙箱实现
- [HTML Entry 源码分析](https://juejin.im/post/6885212507837825038)，详细解读了 HTML Entry 的原理以及在 qiankun 中的应用
...
从 [github](https://github.com/liyongning/qiankun.git) 克隆项目以后，执行一下命令：
...
- 安装示例项目的包
	...
	shellyarn examples:install
	复制代码

以上命令执行结束以后：

![206589898f604e858ab810c1004cf7c2~tplv-k3u1fbpfcp-watermark.image](https://gitee.com/hjb2722404/tuchuang/raw/master/img/ccfd0224b5cfba72c796e6790087e36e.png)
...
### 启动示例项目
	...
	shellyarn examples:start
	复制代码

命令执行结束以后，访问 `localhost:7099` 和 `localhost:7088` 两个地址，可以看到如下内容：

![f5f37a1f6d1f44daa9e332144c0f0c46~tplv-k3u1fbpfcp-watermark.image](https://gitee.com/hjb2722404/tuchuang/raw/master/img/b1af9df0ff004f3fef9c196f7de2b065.png)  ![882deb5b25084476bc9f7fe600ec1f81~tplv-k3u1fbpfcp-watermark.image](https://gitee.com/hjb2722404/tuchuang/raw/master/img/77d1e42041a3d73ce291095691995b74.png)
...
#### main.js
	...
	javascript*// 动态设置 __webpack_public_path__*
	import './public-path';
...
### 应用间通信 initGlobalState
	...
	typescript*// 触发全局监听，执行所有应用注册的回调函数*
...
##### process
	...
	复制代码
	typescriptexport const QiankunCSSRewriteAttr = 'data-qiankun';
	
	复制代码

##### ScopedCSS

	typescript*// https://developer.mozilla.org/en-US/docs/Web/API/CSSRule*
	enum RuleType {
...
以上内容就是对 qiankun 框架的完整解读了，相信你在阅读完这篇文章以后会有不错的收获，源码在 [github](https://github.com/liyongning/qiankun.git)
