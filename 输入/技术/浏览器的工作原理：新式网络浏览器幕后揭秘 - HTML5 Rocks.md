浏览器的工作原理：新式网络浏览器幕后揭秘 - HTML5 Rocks
...
7. **数据存储**。这是持久层。浏览器需要在硬盘上保存各种数据，例如 Cookie。新的 HTML 规范 (HTML5) 定义了“网络数据库”，这是一个完整（但是轻便）的浏览器内数据库。
 ...
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133706.png)
图：浏览器的主要组件。
...
然后进行如下所示的基本流程：
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133714.png)
图：呈现引擎的基本流程。
...
#### 主流程示例
 ...
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133717.png)
图：WebKit 主流程

 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133724.png)
...
示例 - 解析 `2 + 3 - 1` 这个表达式，会返回下面的树：
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133735.png)
 图：数学表达式树节点
...
解析器通常将解析工作分给以下两个组件来处理：**词法分析器**（有时也称为标记生成器），负责将输入内容分解成一个个有效标记；而**解析器**负责根据语言的语法规则分析文档的结构，从而构建解析树。词法分析器知道如何将无关的字符（比如空格和换行符）分离出来。
 ...
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133738.png)
 图：从源文档到解析树
...
很多时候，解析树还不是最终产品。解析通常是在翻译过程中使用的，而翻译是指将输入文档转换成另一种格式。编译就是这样一个例子。编译器可将源代码编译成机器代码，具体过程是首先将源代码解析成解析树，然后将解析树翻译成机器代码文档。
 ...
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133742.png)
图：编译流程
...
可翻译成如下的 DOM 树：
 ...
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133748.png)
 图：示例标记的 DOM 树
...
标记生成器识别标记，传递给树构造器，然后接受下一个字符以识别下一个标记；如此反复直到输入的结束。
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133751.png)
...
现在我们回到**“标记打开状态”**。接收下一个输入字符 `/` 时，会创建 `end tag token` 并改为**“标记名称状态”**。我们会再次保持这个状态，直到接收 `>`。然后将发送新的标记，并回到**“数据状态”**。`</html>` 输入也会进行同样的处理。
 ...
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133756.png)
图：对示例输入进行标记化
...
接收 body 结束标记会触发**“after body”**模式。现在我们将接收 HTML 结束标记，然后进入**“after after body”**模式。接收到文件结束标记后，解析过程就此结束。
 ...
 ![image022.gif](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133802.gif)
图：示例 HTML 的树构建
...
WebKit 使用 [Flex 和 Bison](https://www.html5rocks.com/zh/tutorials/internals/howbrowserswork/#parser_generators) 解析器生成器，通过 CSS 语法文件自动创建解析器。正如我们之前在解析器简介中所说，Bison 会创建自下而上的移位归约解析器。Firefox 使用的是人工编写的自上而下的解析器。这两种解析器都会将 CSS 文件解析成 StyleSheet 对象，且每个对象都包含 CSS 规则。CSS 规则对象则包含选择器和声明对象，以及其他与 CSS 语法对应的对象。
 ...
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133813.png)
图：解析 CSS
...
有一些呈现对象对应于 DOM 节点，但在树中所在的位置与 DOM 节点不同。浮动定位和绝对定位的元素就是这样，它们处于正常的流程之外，放置在树中的其他地方，并映射到真正的框架，而放在原位的是占位框架。
 ...
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133818.png)

图：呈现树及其对应的 DOM 树 ([3.1](https://www.html5rocks.com/zh/tutorials/internals/howbrowserswork/#3_1))。初始容器 block 为“viewport”，而在 WebKit 中则为“RenderView”对象。
...
为了简化样式计算，Firefox 还采用了另外两种树：规则树和样式上下文树。WebKit 也有样式对象，但它们不是保存在类似样式上下文树这样的树结构中，只是由 DOM 节点指向此类对象的相关样式。
 ...
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133829.png)

图：Firefox 样式上下文树 ([2.2](https://www.html5rocks.com/zh/tutorials/internals/howbrowserswork/#2_2))
...
这个想法相当于将规则树路径视为词典中的单词。如果我们已经计算出如下的规则树：
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133833.png)
...
形成的规则树如下图所示（节点的标记方式为“节点名 : 指向的规则序号”）：
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133836.png)
图：规则树

上下文树如下图所示（节点名 : 指向的规则节点）：
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133839.png)
 图：上下文树
...
当呈现器为 dirty 时，会异步触发增量布局。例如，当来自网络的额外内容添加到 DOM 树之后，新的呈现器附加到了呈现树中。
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133845.png)

 图：增量布局 - 只有 dirty 呈现器及其子代进行布局 ([3.6](https://www.html5rocks.com/zh/tutorials/internals/howbrowserswork/#3_6))。
...
每个框都有一个内容区域（例如文本、图片等），还有可选的周围补白、边框和边距区域。
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133900.png)
 图：CSS2 框模型
每一个节点都会生成 0..n 个这样的框。
...
block 框：形成一个 block，在浏览器窗口中拥有其自己的矩形区域。
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133907.png)
 图：block 框
inline 框：没有自己的 block，但是位于容器 block 内。
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133910.png)
图：inline 框
block 采用的是一个接一个的垂直格式，而 inline 采用的是水平格式。
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133914.png)
图：block 和 inline 格式

inline 框放置在行中或“行框”中。这些行至少和最高的框一样高，还可以更高，当框根据“底线”对齐时，这意味着元素的底部需要根据其他框中非底部的位置对齐。如果容器的宽度不够，inline 元素就会分为多行放置。在段落中经常发生这种情况。

 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133916.png)
图：行
...
相对定位：先按照普通方式定位，然后根据所需偏移量进行移动。
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133921.png)
图：相对定位
...
显示效果如下：
 ...
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133924.png)
图：浮动

#### 绝对定位和固定定位

这种布局是准确定义的，与普通流无关。元素不参与普通流。尺寸是相对于容器而言的。在固定定位中，容器就是可视区域。
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133930.png)
图：固定定位
...
这些框分散到多个[堆栈]()（称为堆栈上下文）中。在每一个堆栈中，会首先绘制后面的元素，然后在顶部绘制前面的元素，以便更靠近用户。如果出现重叠，新绘制的元素就会覆盖之前的元素。
...
结果如下：
 ...
 ![](https://gitee.com/hjb2722404/tuchuang/raw/master/img/20201231133934.png)
图：固定定位
