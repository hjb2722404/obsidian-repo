Makefile经典教程(掌握这些足够) - ruglcc's blog - 博客频道 - CSDN.NET
...
Makefile里主要包含了**：显式规则、隐晦规则、变量定义、文件指示和注释。**
...
> 4）如果所有的依赖文件存在或是理当存在，或是就没有依赖文件。那么这条规则将被采用，退出该> [> 算法](http://lib.csdn.net/base/datastructure)> 。
