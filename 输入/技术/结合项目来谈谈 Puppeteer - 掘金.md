...
- 无需人的干预，运行更稳定
- 没有界面，少了真实浏览器加载 css/js 以及渲染页面的工作，无头测试要比真实浏览器更快
...
Pupeeteer 中的基本上所有的操作都是异步的，以上几个 API 都涉及到关于打开一个页面，什么情况下才能判断这个函数执行完毕呢，这些函数都提供了两个参数 waitUtil 和 timeout，waitUtil 表示直到什么出现就算执行完毕，timeout 表示如果超过这个时间还没有结束就抛出异常。
	...
	javascriptawait page.goto('https://www.baidu.com', {
	   timeout: 30 * 1000,
	   waitUntil: [
	       'load',              //等待 “load” 事件触发
...
- page.waitForRequest：等待某个请求出现，返回 Request 实例
	...
	javascriptawait page.waitForXPath('//img');
...
[1](../_resources/ab2fd6e8294833808f4376b893249c1f.webp)
...
在点击一个按钮跳转到新的 Tab 页时会新开一个页面，这个时候我们如何获取改页面对应的 Page 实例呢？可以通过监听 Browser 上的 targetcreated 事件来实现，表示有新的页面创建：
	...
	javascriptlet page = await browser.newPage();
	await page.goto(url);
...
[1](../_resources/d3667f268208cb365e80438dd3669650.webp)

#### Puppeteer 在我们团队的应用场景

[1](../_resources/3cb8f4726f364c3996e84946e1983f14.webp)
...
- [Headless Chrome vs PhantomJS Benchmark](https://hackernoon.com/benchmark-headless-chrome-vs-phantomjs-e7f44c6956c)
- [Scraping iframes with Puppeteer](https://kb.apify.com/en/articles/2116540-scraping-iframes-with-puppeteer)